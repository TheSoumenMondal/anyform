...
type CalendarFormatters = NonNullable<ComponentProps<typeof Calendar>["formatters"]>;
...
const bookedDates: Date[] = Array.from({ length: 3 }, (_, index) => new Date(2025, 5, 17 + index));
...
const calendarFormatters = {
  formatWeekdayName: (date: Date): string => date.toLocaleString("en-US", { weekday: "short" }),
} satisfies CalendarFormatters;

const Calendar24 = () => {
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(initialSelectedDate);
  const [selectedTime, setSelectedTime] = useState<TimeSlot | null>(initialSelectedTime);
        ...
        <CardContent className="relative flex flex-col p-0 max-[1439px]:items-center max-[1439px]:flex-col min-[1440px]:flex-row min-[1440px]:pr-48">
          <div className="p-3 sm:p-4 min-[1440px]:flex-1 min-[1440px]:p-6">
            <Calendar
              ...
              className="bg-transparent! p-0 ring-0! ring-offset-0! focus:ring-0! focus:ring-offset-0! focus-visible:ring-0! focus-visible:ring-offset-0! **:ring-0! **:ring-offset-0! **:focus:ring-0! **:focus-visible:ring-0! [&_.rdp-day_today]:bg-transparent! [--cell-size:--spacing(10)]"
            ...
            {selectedDate && selectedTime ? (
              <>
                ...
                <span>
                  ...
                  <span className="font-medium"> {formatAppointmentDate(selectedDate)} </span>
