...
type TAnimationPreset = keyof typeof cssAnimationPresets;
type TTransitionPreset = keyof typeof cssTransitionPresets;

type TDialogRootProps = ComponentPropsWithoutRef<typeof DialogPrimitive.Root> & {
  modal?: boolean;
};

type TDialogBackdropProps = ComponentPropsWithoutRef<typeof DialogPrimitive.Backdrop> & {
  className?: string;
};

type TDialogPopupProps = ComponentPropsWithoutRef<typeof DialogPrimitive.Popup> & {
  className?: string;
  animationPreset?: TAnimationPreset;
  transitionPreset?: TTransitionPreset;
  children?: ReactNode;
...
const DialogContext = createContext<TDialogContextValue | null>(null);
...
function Dialog({ modal = true, ...props }: TDialogRootProps) {
  return (
...
function DialogViewport(props: ComponentPropsWithoutRef<typeof DialogPrimitive.Viewport>) {
...
function DialogBackdrop({ className = "", ...props }: TDialogBackdropProps) {
  return (
                    ...
                    <DialogPrimitive.Close
                      ...
                      render={
                        <Button
                          variant="outline"
                          size="icon"
                          className="rounded-full size-6 p-0 -mr-4 -mt-2 absolute top-6 right-8"
                        >
...
function DialogFooter({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      ...
      className={cn(
        "flex w-[calc(100%+3rem)] items-center justify-between gap-2 mt-auto -mx-6 rounded-b-2xl border-t border-border/60 bg-muted/50 px-6 py-4",
        className,
      )}
      {...props}
    />
